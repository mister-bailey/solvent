...
class MoleculeProcessor(Process):
    def __init__(self, name, molecule_queue, data_neighbors_queue, \
    ...
    def run(self):
        while True:
            ...
            if work == DatasetSignal.STOP:
                print("molecule processor got a stop")
                self.data_neighbors_queue.put(DatasetSignal.STOP)
                break
            assert isinstance(work, Molecule), \
                   f"expected Molecule but got {type(work)} instead!"
            data_neighbors = self.preprocess_molecule(work)
            for dn in data_neighbors:
                self.data_neighbors_queue.put(dn)

    def preprocess_molecule(self, molecule):
        print(f"{self.name} is processing a molecule")
        data_neighbors = []
